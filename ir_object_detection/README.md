...
## Fixed 2000-frame dataset (frozen)

`manifest_2000.csv` is a **committed, frozen dataset spec**: a fixed set of 2000
frames (≈1760 Phone/Drink objects + ≈240 Safe negatives, balanced across the 5
cameras). The manifest is the source of truth; the images themselves are large
and regenerated from it + the raw videos on demand. Always work on this same set:

```bash
# reproduce the exact 2000 frames (identical every time, any machine):
...
Selection is deterministic, so the manifest can be regenerated byte-identically;
it's committed so the frozen set never drifts even if the sampling logic changes.
To define a *new* frozen set: `--target N --no-extract --freeze-to <new.csv>`,
then commit `<new.csv>`.
